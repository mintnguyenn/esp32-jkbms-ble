...
{
    ESP_LOGE("BleManager", "Resetting state; reason=%d", reason);
}
...
static void bleOnSync(void)
{
    ...
    assert(rc == 0);
    ESP_LOGI("BleManager", "BLE stack synchronized. Scanning can be executed now.");
}
...
esp_err_t BleManager::initialize()
{
    ...
    {
        ESP_LOGE("BleManager", "Failed to init NimBLE");
        return ESP_FAIL;
    }
...
void BleManager::startScanning()
{
    ...
    {
        ESP_LOGE("BleManager", "Error determining address type");
        return;
    }
    ...
    if (rc != 0)
    {
        ESP_LOGE("BleManager", "Failed to start scanning; rc=%d", rc);
    }
}
...
int BleManager::handleEvent(struct ble_gap_event *event)
{
    ...
    {
        ESP_LOGI("BleManager", "Device found: %02x:%02x:%02x:%02x:%02x:%02x",
        ...
        if (memcmp(event->disc.addr.val, TARGET_DEVICE_ADDR, sizeof(TARGET_DEVICE_ADDR)) == 0)
        {
            ...
            {
                ESP_LOGE("BleManager", "Failed to cancel scan");
                return 0;
            }
            ...
            if (rc != 0)
            {
                ESP_LOGE("BleManager", "Connection failed; rc=%d", rc);
            }
        }
        break;
    }
    ...
    case BLE_GAP_EVENT_CONNECT:
    {
        ...
        ESP_LOGI("BleManager", "CONNECTION COMPLETED; conn_handle=%d", event->connect.conn_handle);
        int rc = ble_gattc_exchange_mtu(event->connect.conn_handle, nullptr, nullptr);
        if (rc != 0)
        {
            ESP_LOGW("BleManager", "MTU exchange failed; rc=%d", rc);
        }
        break;
    }
    ...
    case BLE_GAP_EVENT_NOTIFY_RX:
    {
        ...
        // Uncomment the next line for hex dump debugging:
    ...
    {
        ESP_LOGI("BleManager", "MTU update event; conn_handle=%d, cid=%d, mtu=%d",
...
bool BleManager::write_register(uint8_t address, uint32_t value, uint8_t length)
{
    ...
    if (rc != 0)
    {
        ESP_LOGW("BleManager", "write_register failed; rc=%d", rc);
    }
    return rc == 0;
}
...
{
    ESP_LOGI("BleManager", "BLE Host Task Started");
    nimble_port_run();
